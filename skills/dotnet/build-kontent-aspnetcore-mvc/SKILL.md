...
Target the current major line of every Kontent.ai package: Delivery 20.x, AspNetCore 1.x and ModelGenerator 11.x on `net10.0`. While that line is in release candidate, pass `--prerelease` to `dotnet add package` and `dotnet tool install`; a release candidate of the current line beats a stable release of the previous major, because this skill's API guidance is written for it. Fall back to the previous stable line only when the app must stay on `net8.0`, and say which line was chosen in the report.
